...
The reference implementation is available as \texttt{crdt-merge} v0.9.5, which additionally includes E4 Recursive Trust-Delta Entanglement -- a protocol that makes trust a first-class CRDT dimension, enabling Byzantine detection without consensus (Section~\ref{sec:e4}).
\end{abstract}
\vspace{1.5em}
]
...
To validate convergence under realistic distributed conditions, we execute a four-part convergence suite using the \texttt{crdt-merge} library (v0.9.5).
...
The architecture did not originally provide Byzantine fault tolerance; this has been addressed by E4 (Section~\ref{sec:e4}), implemented in v0.9.5.
The two-layer separation enabled a trust-as-CRDT extension where trust metadata propagates through the same lattice semantics as data.
This has been implemented as the E4 protocol (Section~\ref{sec:e4}), which extends the architecture to a four-dimensional product lattice with Byzantine detection through trust score convergence.
See Section~\ref{sec:e4} for details.
Integration with existing Byzantine-resilient aggregation methods~\cite{blanchard2017machine} remains possible as a complementary defence layer.


% ============================================================
% 8. E4: TRUST AS A CRDT DIMENSION
% ============================================================
\section{E4: Trust as a CRDT Dimension}\label{sec:e4}

The limitation identified in Section~\ref{sec:limitations} (L4) -- that the architecture provides no Byzantine fault tolerance -- motivated E4 Recursive Trust-Delta Entanglement, implemented in v0.9.5.
E4 extends the two-layer architecture without modifying it: trust becomes an additional dimension of the CRDT state, entangled with data at the lattice level.

\subsection{Product Lattice Structure}

E4 extends \CMS{} from a three-component tuple $(A, R, V, H)$ to a four-dimensional product lattice:

\begin{equation}\label{eq:e4state}
\mathrm{E4State} = \mathrm{Data} \times \mathrm{Trust} \times \mathrm{Clock} \times \mathrm{Hash}
\end{equation}

Each dimension is a join-semilattice.
The product of join-semilattices is a join-semilattice~\cite{shapiro2011comprehensive}.
The join is defined dimension-by-dimension:

\begin{equation}\label{eq:e4join}
(d_1, t_1, c_1, h_1) \sqcup (d_2, t_2, c_2, h_2) = (d_1 \sqcup d_2,\; t_1 \sqcup t_2,\; c_1 \sqcup c_2,\; \mathrm{recompute}(h))
\end{equation}

Convergence follows from the algebraic structure -- it is inherited, not tested.
The Data dimension join is unchanged from Eq.~\ref{eq:merge}, so enabling E4 cannot break existing convergence guarantees.

\subsection{Six Foundational Primitives}

E4 is built from six primitives:

\begin{enumerate}[leftmargin=*]
\item \textbf{Change detection (P1).} Given two states, identify what differs -- implemented through 256-ary Merkle tree differencing.
\item \textbf{Change encoding (P2).} Represent differences compactly -- projection delta encoding extracts only changed subtrees and compresses through parameter-aware encoding.
\item \textbf{Change verification (P3).} Prove differences are legitimate -- each delta carries a cryptographic proof that is verified independently of trust scores.
\item \textbf{Trust assignment (P4).} Score each peer's reliability -- a five-dimensional trust vector (integrity, causality, consistency, gossip, model quality) maintained as per-dimension GCounters with homeostatic budget normalisation.
\item \textbf{Trust-gated application (P5).} Apply changes only from sufficiently trusted peers -- a configurable threshold gates data application at the Layer~2 boundary.
\item \textbf{Recursive propagation (P6).} Trust changes propagate as data changes through the same delta pipeline they govern.
\end{enumerate}

P1--P2 correspond to delta-state CRDTs~\cite{almeida2018delta}.
P3 corresponds to authenticated data structures.
P4--P5 correspond to Byzantine filtering, established in consensus protocols but not previously as CRDT-native semantics.
P6 -- recursive trust-delta propagation -- is the binding contribution: when a peer's trust score changes, that change is encoded as a delta, transmitted and verified through the same pipeline as data deltas.

\subsection{Symbiotic Lattice Trust (SLT) Protocol}

The SLT protocol implements Byzantine detection through trust lattice convergence rather than voting rounds.
Each peer maintains a per-peer trust vector as five independent GCounters.
Evidence of malicious behaviour -- equivocation (sending inconsistent states to different peers), Merkle-root divergence, clock regression, invalid deltas, or trust manipulation -- triggers monotonic trust decay.
Because GCounters are CRDTs, all honest peers converge on the same trust state without coordination.

SLT's threat model differs from classical BFT~\cite{castro1999practical}.
Classical BFT guarantees safety below $n/3$ Byzantine actors through voting rounds.
SLT operates without voting -- Byzantine detection emerges from trust score convergence across the lattice, and trust-gated application at the Layer~2 boundary excludes contributions from peers whose converged trust falls below threshold.
The 34\% tolerance threshold was measured empirically with actively Byzantine peers performing data injection, trust inflation, and selective withholding.
End-to-end federation latency across 10 nodes with 2 actively Byzantine peers: 9.69\,ms.

\subsection{Delta Encoding}

Projection delta encoding decomposes state changes through 256-ary Merkle tree differencing, extracts only changed subtrees, and compresses through parameter-aware encoding.
Deltas compose associatively -- chaining any sequence produces the same result as a single diff between endpoints.
Validated on \texttt{facebook/opt-6.7b} (6.7 billion parameters).
Causal clock throughput: 2.93\,M operations per second.
Proof-carrying operation wire size: 128 bytes fixed.

\subsection{Backward Compatibility}

E4 activates transparently.
Every existing API call works identically.
The product lattice join is defined dimension-by-dimension, and the Data dimension join is unchanged.
Disable with \texttt{CRDT\_MERGE\_E4=0} if needed.
...
v0.9.5 implements E4 Recursive Trust-Delta Entanglement (Section~\ref{sec:e4}), which resolves the Byzantine limitation (L4) by making trust a first-class CRDT dimension with lattice-native Byzantine detection.
Remaining future directions include cross-hardware determinism validation, incremental resolve strategies for large contribution sets, and integration with federated learning pipelines.
...
The \texttt{crdt-merge} library (v0.9.5) and accompanying verification notebook are available at \url{https://github.com/RyanGillespie/crdt-merge}.
...
All experiments use the \texttt{crdt-merge} library v0.9.5.
