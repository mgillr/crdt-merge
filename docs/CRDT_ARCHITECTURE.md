...
### 3.1 Architecture 1: G-Set Accumulator

**Concept**: Use a Grow-only Set (G-Set) — the simplest CRDT — to
accumulate model contributions. Elements can only be added, never
removed. The merge operation is set union.

```
GSetState = { model_A, model_B, model_C, ... }
merge(S₁, S₂) = S₁ ∪ S₂
resolve(S) = strategy(sorted(S))
```

**Strengths**:
- Simplest possible CRDT
- Set union trivially satisfies C+A+I
- Easy to implement and reason about

**Weaknesses**:
- No ability to remove models once added (no "undo")
- State grows monotonically
- No support for model versioning or replacement

**Test Result**: 25/25 strategies passed CRDT compliance. ✓

### 3.2 Architecture 2: Delta-State CRDT

**Concept**: Store model contributions as *deltas* (task vectors) from
a shared base model. The CRDT state is a delta map; the merge operation
is map union with per-key join.

```
DeltaState = { id → (base_hash, delta_tensor, metadata) }
merge(D₁, D₂) = { k: join(D₁[k], D₂[k]) for k in D₁.keys() ∪ D₂.keys() }
resolve(D) = strategy(base + deltas)
```

**Strengths**:
- Storage-efficient (deltas are often sparse)
- Natural fit for task-arithmetic family of strategies
- Supports efficient network transfer

**Weaknesses**:
- Requires agreement on a common base model
- Join function for conflicting deltas requires careful design
- More complex than G-Set

**Test Result**: 25/25 strategies passed CRDT compliance. ✓

### 3.3 Architecture 3: Monoid Accumulator

**Concept**: Embed each model contribution into a free monoid (sequence
of elements). The merge operation is the monoidal product (concatenation
with deduplication). Resolution flattens the monoid into a single model.

```
MonoidState = [contribution₁, contribution₂, ...]
merge(M₁, M₂) = deduplicate(M₁ ++ M₂)
resolve(M) = strategy(canonicalize(M))
```

**Strengths**:
- Algebraically principled
- Preserves order of contributions for strategies that need it
- Natural composition via monoidal product

**Weaknesses**:
- Deduplication must be carefully implemented to preserve idempotency
- Canonicalization adds complexity
- Theoretical elegance doesn't always translate to practical simplicity

**Test Result**: 25/25 strategies passed CRDT compliance. ✓

### 3.4 Architecture 4: Canonical Order

**Concept**: Store contributions in a set and use a deterministic,
content-based ordering (e.g., SHA-256 hash of serialized tensors) to
establish a canonical processing order. All replicas sort identically
before applying the strategy.

```
CanonicalState = { (hash, model, metadata), ... }
merge(C₁, C₂) = C₁ ∪ C₂
resolve(C) = strategy(sort_by_hash(C))
```

**Strengths**:
- Simple and predictable
- Hash-based ordering is deterministic across all replicas
- Works well with strategies sensitive to input order

**Weaknesses**:
- Hash computation on large models is expensive
- Ordering is arbitrary (not semantically meaningful)
- Essentially a refinement of G-Set with ordering guarantees

**Test Result**: 25/25 strategies passed CRDT compliance. ✓

### 3.5 Architecture 5: Merkle-DAG

**Concept**: Build a content-addressable Merkle DAG (Directed Acyclic
Graph) of model contributions and their provenance. Each node contains
a model snapshot and pointers to its parents. The merge operation is
DAG union.

```
MerkleNode = { hash, tensor_data, parent_hashes[], metadata }
MerkleDAG = { hash → MerkleNode }
merge(G₁, G₂) = G₁ ∪ G₂  (node-level union)
resolve(G) = strategy(leaf_nodes(G))
```

**Strengths**:
- Full provenance tracking (who merged what, when)
- Content-addressable — automatic deduplication
- Efficient sync (only transfer missing nodes)
- Natural fit for version-control–like workflows

**Weaknesses**:
- Most complex architecture
- DAG traversal and leaf-node identification add overhead
- Provenance graph can grow large over many merge generations

**Test Result**: 25/25 strategies passed CRDT compliance. ✓

### 3.6 Architecture 6: Hybrid Two-Phase OR-Set

**Concept**: Use an Observed-Remove Set (OR-Set) — a well-studied CRDT
that supports both add and remove operations via unique tags and
tombstones. Each model contribution is tagged with a unique identifier;
removals record the tag in a tombstone set.

```
ORSetState = {
    adds:    { (unique_tag, model_id, tensor_data, metadata), ... },
    removes: { unique_tag, ... }   # tombstones
}
visible(S) = { e ∈ S.adds : e.tag ∉ S.removes }
merge(S₁, S₂) = ORSet(adds = S₁.adds ∪ S₂.adds,
                        removes = S₁.removes ∪ S₂.removes)
resolve(S) = strategy(visible(S))
```

**Strengths**:
- Supports both add and remove (model retirement/replacement)
- Well-studied CRDT with known correctness proofs
- Two-phase design cleanly separates state management from execution
- Version vectors enable causal ordering

**Weaknesses**:
- Tombstones accumulate (require periodic garbage collection)
- Slightly more complex than G-Set
- Unique tag generation requires care in distributed settings

**Test Result**: 25/25 strategies passed CRDT compliance. ✓

### 3.7 Architecture 7: Lattice Consensus

**Concept**: Define a join-semilattice directly over the space of "merge
configurations" — not over raw tensors, but over the metadata and
parameterization of merge operations. The lattice join accumulates
knowledge about which models and parameters should participate.

```
LatticeState = {
    participants: PowerSet(Models),      # ⊔ = union
    weights: Map(Model → MaxRegister),   # ⊔ = max
    config: Map(Key → LWWRegister),      # ⊔ = last-writer-wins
}
merge(L₁, L₂) = componentwise_join(L₁, L₂)
resolve(L) = strategy(L.participants, L.weights, L.config)
```

**Strengths**:
- Most theoretically rigorous
- Each component of state uses an appropriate CRDT type
- Fine-grained conflict resolution (per-weight, per-config)

**Weaknesses**:
- Most complex to implement correctly
- LWW registers require synchronized clocks or logical timestamps
...
| Architecture | Complexity | Add/Remove | Provenance | Sync Efficiency | Production Ready |
|-------------|:----------:|:----------:|:----------:|:---------------:|:---------------:|
| G-Set | ★☆☆☆☆ | Add only | ✗ | ★★★☆☆ | ✓ |
| Delta-State | ★★★☆☆ | Add only | Partial | ★★★★☆ | ✓ |
| Monoid | ★★★☆☆ | Add only | ✗ | ★★★☆☆ | ✓ |
| Canonical Order | ★★☆☆☆ | Add only | ✗ | ★★★☆☆ | ✓ |
| Merkle-DAG | ★★★★★ | Add only | ✓ | ★★★★★ | ✓ |
| **OR-Set (Hybrid)** | **★★★☆☆** | **Add+Remove** | **Partial** | **★★★★☆** | **✓** |
| Lattice Consensus | ★★★★★ | Add+Remove | ✓ | ★★★☆☆ | ✓ |

### 3.9 Why OR-Set Won

The **Hybrid Two-Phase OR-Set** (Architecture 6) was selected as the
production architecture for the following reasons:

1. **Add and Remove support** — Unlike G-Set, Delta-State, Monoid, and
   Canonical Order, the OR-Set supports removing model contributions,
   which is essential for practical workflows (retiring bad models,
   replacing outdated fine-tunes).

2. **Simplicity vs. Power** — It achieves the same CRDT guarantees as
   the Lattice Consensus approach with significantly less implementation
   complexity.
...
5. **Merkle hashing** — Content-addressable hashing from Architecture 5
   was incorporated for efficient deduplication and integrity checking.

6. **Version vectors** — Causal ordering from Architecture 7 was
   incorporated for conflict-free distributed operation.

The production `CRDTMergeState` class combines the best ideas from all
seven architectures into a cohesive, battle-tested implementation.
