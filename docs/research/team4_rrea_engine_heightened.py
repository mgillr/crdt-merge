...
def main():
    ...
    high_shannon = [(s, h) for s, h in sorted(shannon.items(), key=lambda x: -x[1]) if h > 0.15]
    ...
    high_ping = [(s, h) for s, h in sorted(ping_entropy.items(), key=lambda x: -x[1]) if h > 0.15]
